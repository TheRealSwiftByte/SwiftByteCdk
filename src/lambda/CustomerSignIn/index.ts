...
export async function handler(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
    try {
        ...
        const customerScan: ScanCommandInput = {
        ...
        const results = await client.send(new ScanCommand(customerScan));

        if (!results.Items || results.Items.length === 0) {
            throw new Error('No customer found with provided login details');
        }

        if (results.Items.length > 1) {
            throw new Error('Multiple customers found with provided login details');
        }


        const customerDynamo = results.Items[0];
        const customer = DynamoToObject(customerDynamo);
